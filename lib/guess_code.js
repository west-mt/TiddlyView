...
function GetEscapeCodeType(str){
  ...
  //if(/%([0-9A-DF][0-9A-F]%[8A]0%|E0%80|[0-7][0-9A-F]|C[01])%[8A]0|%00|%[7F]F/i.test(str))return "UTF-16LE_";
  if(/%E[0-9A-F]%[8A]0%[8A]0|%[CD][0-9A-F]%[8A]0/i.test(str))return "UTF-8";
  if(/%F[DE]/i.test(str))return /%8[0-9A-D]|%9[0-9A-F]|%A0/i.test(str)?"UTF16-LE_":"EUC-JP";
  if(/%1B/i.test(str))return /%[A-D][0-9A-F]/i.test(str)?"JIS-8":"JIS-7";
  ...
  while(0<=(c=parseInt(S.substring(i,i+=2),16))&&i<4092)if(128<=c){
	if((C=parseInt(S.substring(i,i+2),16))<128)i+=2;
	  ...
	  if(c<224){T="UTF-8";i+=2;continue;}
	  if(2==parseInt(S.charAt(i+2),16)>>2){T="UTF-8";i+=4; continue; }
	}
	if(142==c&&161<=C&&C<224){
	  if(!T) T="EUC-JP";
	  if("EUC-JP"==T) continue;
	}
	if(c<161)return "Shift_JIS";
	if(c<224&&!T)
	  if((164==c&&C<244||165==c&&C<247)&&161<=C)i+=2;
	else T=224<=C?"EUC-JP":"Shift_JIS";
	else T="EUC-JP";
  }
  return T?T:"EUC-JP";
};
...
function guess_code(array) {
  ...
  cs=c===239&&array[1]===187&&array[2]===191?"UTF-8":c===255&&array[1]===254?"UTF-16LE":c===254&&array[1]===255?"UTF-16BE":null;
  ...
  if (cs==="ASCII") {
    cs = u7&&(b64>0||pm>0) ? "UTF-7" : mu7&&(mb64>0||am>0) ? "MUTF-7" : "ASCII";
  }
