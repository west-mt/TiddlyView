  ...
  block: {
	...
	(function( ) {
       ...
       return function( block, next ) {
		 //print(JSON.stringify("block: "+block));
		 //print(JSON.stringify("next : "+next));
         ...
         for(var line_no = 0; line_no < lines.length; line_no++) {
		   ...
		   if(m){
			 ...
			 if(m[2] !== undefined){
			   //print('m[2]: '+m[2].length);
			   if(m[2].length >= last_depth) depth = -1;
			 }
			 ...
			 if(depth > last_depth){
			   ...
			   if(current_list[0] != list_type){
				 ...
				 nest_stack[nest_stack.length-1].push(new_list);
				 nest_stack.push(new_list);
		 ...
		 result[result.length-1].splice(1, 0, {'class': 'top'});
...
// Split source into rough blocks
